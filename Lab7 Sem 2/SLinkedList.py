...
class SLinkedList:
    ...
    def insert(self,pos,item):
        assert type(pos) == int, "Must be an integer"
        assert pos >= 0, "Must be non negative"
        
        if pos == 0: #Checks to see if the position is 0
            x = SLinkedListNode(item, self.head) #Creates a default node
            self.head = x #Adds the node to the head of the list
            self.size += 1 #Increase the list size by 1
            return None
 
        start = self.head 
        while pos > 1: #If the position is not 0
            start = start.next #Move through the list
            pos -= 1 #Position is now closer to 0
        start.next = SLinkedListNode(item, start.next) 
        self.size += 1 #Increase the list size by 1
