...
def dequeue_experiment(queue):
    ...
    begin = time.time() #Start of queue time
    while not queue.isEmpty(): #Loop for a non empty queue
        queue.dequeue() #Dequeue elements
    end = time.time() #End time for dequeue
    times = times + end - begin #Calculation for the total queue time
    return times
...
def avg_dequeue_experiment(queue_class, size, sample_size = 5):
    ...
    '''
    avg = 0 #Initialized queue variabale
    for i in range(sample_size): #Loop for elements in the range of the sample
        queue = enqueue_experiment(queue_class, size) #Enqueues the class and size of the queue
        avg += dequeue_experiment(queue) #Gets the average time to dequeue the sample size

    return float(avg)/sample_size #Calculates the average time taken for the queue
