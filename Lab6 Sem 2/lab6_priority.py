...
def main():
    ...
    for t in range(time_steps):
        ...
        ########################
        if not process_running: #Checks to see if the process is not running
            if not high_priority_queue.isEmpty(): #Checks to see if the high priority queue is empty
                current_job = high_priority_queue.dequeue() #If the above passed then the current job is dequeueing the highpriority elements
            elif not low_priority_queue.isEmpty(): #Checks to see if the low priority queue is empty
                current_job = low_priority_queue.dequeue() #If the above passed the current job is to dequeue the lowpriority elements
